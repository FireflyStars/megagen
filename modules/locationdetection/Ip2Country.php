...
class Ip2Country
{
	...
	private function appendToFile($db)
	{
		...
		{			
			$filename = dirname(__FILE__) .$this->dir . $piece . '.php';
	...
	public function parseCSV($filename = 'IpToCountry.csv')
	{
		...
		foreach ($db AS $piece=>$entries)
		{ 
			$f = fopen(dirname(__FILE__) .$this->dir . $piece . '.php', 'w');
			fputs($f, '<?php $entries = array(' . "\n");
	...
	public function parseCSV2($filename = 'IpToCountry.csv')
	{
		...
		{
		    if ($dh = opendir(dirname(__FILE__) .$this->dir)) 
			{
		        while (($file = readdir($dh)) !== false) 
				{
					...
					$this->finishFile(dirname(__FILE__) .$this->dir . $file);   
		        }
		        closedir($dh);
		    }
		}
	...
	public function load($ip)
	{
		...
		$piecem1 = (int)$piece-1;
		$piecep1 = (int)$piece+1;
		if (file_exists(dirname(__FILE__) .$this->dir . $piece . '.php'))
		{
			include dirname(__FILE__) .$this->dir . $piece . '.php';
		...
		if (file_exists(dirname(__FILE__) .$this->dir . $piecem1 . '.php'))
		{
			include dirname(__FILE__) .$this->dir . $piecem1 . '.php';
			foreach ($entries AS $e)
			{	
				$e[0] = floatval($e[0]);
		...
		if (file_exists(dirname(__FILE__) .$this->dir . $piecep1 . '.php'))
		{
			include dirname(__FILE__) .$this->dir . $piecep1 . '.php';
			foreach ($entries AS $e)
			{	
				$e[0] = floatval($e[0]);
