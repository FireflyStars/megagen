...
class ThemeConfigurator extends Module
{
	...
	protected function updateItem()
	{
		...
		if (!Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'themeconfigurator` SET
					...
					image_w = '.(int)$image_w.',
					image_h = '.(int)$image_h.',
					discount = '.(float)Tools::getValue('item_discount').',
	...
	protected function addItem()
	{
		...
		if (!Db::getInstance()->Execute('
			INSERT INTO `'._DB_PREFIX_.'themeconfigurator` (
					`id_shop`, `id_lang`, `item_order`, `title`, `title_use`, `hook`, `url`, `target`, `image`, `image_w`, `image_h`, `discount`,`html`, `active`
			) VALUES (
					...
					\''.pSQL($image_w).'\',
					\''.pSQL($image_h).'\',
					\''.(float)Tools::getValue('item_discount').'\',
