...
class AwsLib
{
    ...
    public function getListDirectories($path = '', $next_continuation_token = '')
    {
        ...
        if (!$result['success'] || !isset($result['result']['CommonPrefixes'])) {
            return false;
        }
    ...
    public function completeMultipartUpload($destination, $upload_id, $filesize, $list_parts)
    {
        ...
        if ($result['success'] === false || $result['code_http'] != '200') {
            $this->log(
                'WAR'
                .sprintf($this->ntbr->l('The %s upload of the file %s cannot be completed', self::PAGE), NtbrChild::AWS, $destination)
            );
            return false;
        }
    ...
    /*public function fetchQuota()
    {
        ...
        $this->log(
            sprintf($this->ntbr->l(
                'Sending to %s account:', self::PAGE), NtbrChild::AWS)
            .' '.$this->ntbr->l(
