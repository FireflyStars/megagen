    ...
    if ($ftp_login && $ftp_login != '') {
        $insert_into_table_ntbr_ftp = Db::getInstance()->execute('
            INSERT INTO '._DB_PREFIX_.'ntbr_ftp
                (`active`, `name`, `nb_backup`, `sftp`, `ssl`, `passive_mode`, `server`, `login`, `password`, `port`,
                `directory`, `date_add`, `date_upd`)
            VALUES ('.(int)$send_ftp.', "FTP 1", '.(int)$nb_keep_backup_ftp.', '.(int)$send_sftp.', '.(int)$ftp_ssl.', '
            .(int)$ftp_pasv.', "'.pSQL($ftp_server).'", "'.pSQL($ftp_login).'", "'.pSQL($ftp_pass).'", '.(int)$ftp_port
            .', "'.pSQL($ftp_dir).'", "'.pSQL($now).'", "'.pSQL($now).'")
        ');
    ...
    if ($dropbox_access_token && $dropbox_access_token != '') {
        $insert_into_table_ntbr_dropbox = Db::getInstance()->execute('
            INSERT INTO '._DB_PREFIX_.'ntbr_dropbox
                (`active`, `name`, `nb_backup`, `directory`, `token`, `date_add`, `date_upd`)
            VALUES ('.(int)$send_dropbox.', "Dropbox 1", '.(int)$nb_keep_backup_dropbox.', "'.pSQL($dropbox_dir).'", "'
            . pSQL($dropbox_access_token).'", "'.pSQL($now).'", "'.pSQL($now).'")
        ');
    ...
    if ($owncloud_user && $owncloud_user != '') {
        $insert_into_table_ntbr_owncloud = Db::getInstance()->execute('
            INSERT INTO '._DB_PREFIX_.'ntbr_owncloud
                (`active`, `name`, `nb_backup`, `login`, `password`, `server`, `directory`, `date_add`, `date_upd`)
            VALUES ('.(int)$send_owncloud.', "ownCloud 1", '.(int)$nb_keep_backup_owncloud.', "'.pSQL($owncloud_user)
            .'", "'. pSQL($owncloud_pass).'", "'. pSQL($owncloud_server).'", "'. pSQL($owncloud_dir).'", "'
            .pSQL($now).'", "'.pSQL($now).'")
        ');
    ...
    if ($googledrive_access_token && $googledrive_access_token != '') {
        $insert_into_table_ntbr_googledrive = Db::getInstance()->execute('
            INSERT INTO '._DB_PREFIX_.'ntbr_googledrive
                (`active`, `name`, `nb_backup`, `directory_key`, `directory_path`, `token`, `date_add`, `date_upd`)
            VALUES ('.(int)$send_googledrive.', "Google Drive 1", '.(int)$nb_keep_backup_googledrive.', "'
            .pSQL($googledrive_dir).'", "'. pSQL($googledrive_dir_path).'", "'. pSQL($googledrive_access_token).'", "'
            .pSQL($now).'", "'.pSQL($now).'")
        ');
    ...
    if ($onedrive_access_token && $onedrive_access_token != '') {
        $insert_into_table_ntbr_onedrive = Db::getInstance()->execute('
            INSERT INTO '._DB_PREFIX_.'ntbr_onedrive
                (`active`, `name`, `nb_backup`, `directory_key`, `directory_path`, `token`, `date_add`, `date_upd`)
            VALUES ('.(int)$send_onedrive.', "OneDrive 1", '.(int)$nb_keep_backup_onedrive.', "'.pSQL($onedrive_dir)
            .'", "'. pSQL($onedrive_dir_path).'", "'. pSQL($onedrive_access_token).'", "'
            .pSQL($now).'", "'.pSQL($now).'")
        ');
