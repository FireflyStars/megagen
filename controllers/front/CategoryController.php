...
class CategoryControllerCore extends FrontController
{
	...
	public function canonicalRedirection($canonical_url = '')
	{
		...
		// hack to show the product home category as shop start point /2-home
	...
	public function initContent()
	{
		...
		$categoryid = (int)$this->category->id;
		 $sql = 'SELECT * FROM `'._DB_PREFIX_.'group_reduction` as p where id_category = "'.$categoryid.'"';
        $categorydiscounts = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
        $displaydiscount = '';
        if(count($categorydiscounts) > 0){
        	$displaydiscount = $categorydiscounts[0]['reduction'] * 100;
        }
