...
class AuthControllerCore extends FrontController
{
	...
	public function init()
	{
		...
		if (!Tools::getIsset('step') && $this->context->customer->isLogged() && !$this->ajax)
			...
			// Tools::redirect('/2-home');
				// Tools::redirect("content/39-new-customer-waiting-approval");
			...
			if (!count($this->errors))
			{
				...
				if (!count($this->errors))
				{
					if ($customer->add())
					{
						...
						// }
						// else : redirection to the account (redirect to waiting approval)
							// $approvalLink = $this->context->link->getCMSLink(39);
						// else {
							// $approvalLink = "content/39-new-customer-waiting-approval";
							// Tools::redirect($approvalLink);
							Tools::redirect('index.php?controller='.(($this->authRedirection !== false) ? urlencode($this->authRedirection) : 'my-account'));
						// }
					}
					else
				...
				else
				{
					...
					if (!count($this->errors))
					{
						...
						// else
							// $approvalLink = "content/39-new-customer-waiting-approval";
							// Tools::redirect($approvalLink);
							Tools::redirect('index.php?controller='.(($this->authRedirection !== false) ? urlencode($this->authRedirection) : 'shop'));
					}
				}
			}
		}
